...
This repository includes `render.yaml`, which defines two Render web services:

- `medical-tailoring-api`: FastAPI backend with local ChromaDB and uploaded image storage.
- `medical-tailoring-ui`: Streamlit frontend connected to the API service.

1. Push the repository to GitHub.
2. In Render, choose **New +** and then **Blueprint**.
3. Connect the `San011989/Medical_Afzal` GitHub repository.
4. Select the `main` branch and apply `render.yaml`.
5. Enter these secret values when Render prompts for them on the API service:

```text
OPENAI_API_KEY=your_replacement_openai_api_key
AUTH_USERNAME=your_private_username
AUTH_PASSWORD=your_private_password
```

`JWT_SECRET_KEY` is generated by Render automatically. The UI receives the API hostname from `render.yaml` through `BACKEND_URL`.

After both services deploy, open the Streamlit UI service URL. Use the private `AUTH_USERNAME` and `AUTH_PASSWORD` values to sign in.

### Free Render plan limitation

The free plan does not include persistent disks. This project can still deploy for free, but ChromaDB records and uploaded diagrams stored on the API filesystem may be deleted when the service restarts, sleeps, or redeploys. For permanent data, use a paid persistent disk or move storage to an external database/object-storage service.

Never commit `.env`, API keys, or real passwords. The local `.env` file is ignored by git, and Render secrets should be entered in the Render dashboard rather than in `render.yaml`.
